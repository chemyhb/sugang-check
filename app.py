...
def create_categorized_checkbox_group(group_id, limit, categorized_subjects):
    ...
    for category, subjects in categorized_subjects.items():
        st.markdown(f"**🔹 {category}**") # 교과군 소제목
        cols = st.columns(2)
...
with col1:
    with st.container(border=True):
        ...
        create_categorized_checkbox_group("2-1-B", 5, {
            "국어/수학/영어": [('독서 토론과 글쓰기', '진로'), ('인공지능 수학', '진로'), ('세계 문화와 영어', '융합')],
            "사회": [('현대사회와 윤리', '일반'), ('세계시민과 지리', '일반'), ('정치', '일반'), ('동아시아 역사 기행', '진로')],
            "과학": [('물리학', '일반'), ('화학', '일반'), ('생명과학', '일반')],
            "기술·가정 / 제2외국어 / 교양": [('기술·가정', '일반'), ('지식 재산 일반', '융합'), ('정보', '일반'), ('일본어', '일반'), ('중국어', '일반'), ('한문', '일반')]
        })
        st.divider()
...
with col2:
    with st.container(border=True):
        ...
        create_categorized_checkbox_group("2-2-D", 5, {
            "국어/수학/영어": [('언어생활 탐구', '융합'), ('기하', '진로'), ('경제 수학', '진로'), ('미디어 영어', '융합')],
            "사회": [('세계사', '일반'), ('인문학과 윤리', '융합'), ('법과 사회', '일반'), ('경제', '일반'), ('한국지리 탐구', '진로')],
            "과학": [('지구과학', '일반'), ('역학과 에너지', '진로'), ('화학 반응의 세계', '진로'), ('생물의 유전', '진로')],
            # 생활과학 탐구를 아래 기술·가정 교과군으로 올바르게 이동했습니다.
            "기술·가정 / 제2외국어 / 교양": [('생활과학 탐구', '융합'), ('로봇과 공학세계', '진로'), ('인공지능 기초', '진로'), ('일본어 회화', '진로'), ('중국어 회화', '진로')]
        })
        st.divider()
        st.write("📌 **[택 1]** 예술 2학점 선택")
...
with col3:
    with st.container(border=True):
        ...
        create_categorized_checkbox_group("3-1-E", 5, {
            "국어/수학": [('문학과 영상', '융합'), ('미적분Ⅱ', '진로'), ('수학과제 탐구', '진로')],
            "사회": [('사회와 문화', '일반'), ('윤리와 사상', '일반')],
            "과학": [('물질과 에너지', '진로'), ('전자기와 양자', '진로'), ('세포와 물질대사', '진로'), ('지구시스템과학', '진로'), ('행성우주과학', '진로'), ('융합과학 탐구', '융합')],
            "기술·가정 / 제2외국어 / 교양": [('생활과학 탐구', '융합'), ('로봇과 공학세계', '진로'), ('데이터 과학', '진로'), ('심화 일본어', '진로'), ('심화 중국어', '진로')]
        })
        st.divider()
        st.write("📌 **[택 1]** 교양 2학점 선택")
        create_categorized_checkbox_group("3-1-F", 1, {
            "교양/예술": [('보건', '교양'), ('인간과 철학', '교양'), ('생태와 환경', '교양'), ('미술 감상과 비평', '교양')]
        })
...
with col4:
    with st.container(border=True):
        ...
        create_categorized_checkbox_group("3-2-G", 8, {
            "국어/수학/영어": [('주제 탐구 독서', '진로'), ('매체 의사소통', '진로'), ('수학과 문화', '융합'), ('실용 통계', '융합'), ('심화 영어', '진로'), ('영미 문학 읽기', '진로')],
            "사회": [('기후변화와 지속가능한 세계', '융합'), ('역사로 탐구하는 현대 세계', '진로'), ('여행지리', '진로'), ('윤리문제 탐구', '진로'), ('사회문제 탐구', '진로')],
            "과학": [('기후변화와 환경생태', '융합'), ('과학의 역사와 문화', '융합'), ('융합과학 탐구', '융합')],
            "체육/예술": [('운동과 건강', '일반'), ('음악 감상과 비평', '진로')],
            "기술·가정 / 교양": [('아동발달과 부모', '융합'), ('지식 재산 일반', '융합'), ('소프트웨어와 생활', '융합'), ('일본 문화', '융합')]
        })
        st.divider()
        st.write("📌 **[택 1]** 교양/실무 2학점 선택")
        create_categorized_checkbox_group("3-2-H", 1, {
            "교양": [('인간과 심리', '교양'), ('교육의 이해', '교양'), ('인간과 경제활동', '교양'), ('논리와 사고', '교양'), ('생애 설계와 자립', '교양')]
        })
...
if st.button("🔍 수강신청 규정 검증하기", use_container_width=True):
    if not st.session_state.st_id or not st.session_state.st_name:
        ...
        for idx, (sem, subjs) in enumerate(selected_by_semester.items()):
